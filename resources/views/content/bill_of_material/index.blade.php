                ...
                <div class="filter-card">
                    <form action="{{ route('bill_of_materials') }}" method="GET">
                        <div class="row g-3">
                            ...
                            <div class="col-md-2">
                                <select class="form-select" name="status" aria-label="Filter by Product Status">
                                    <option value="" {{ ($currentStatus ?? '') === '' ? 'selected' : '' }}>All Product
                                        Statuses</option>
                                    ...
                                    <div class="card-body">
                                        <div class="d-flex justify-content-between align-items-start mb-3">
                                            ...
                                            @if ($product->billOfMaterial()->where('is_active', true)->exists())
                                                <span class="badge bg-success-subtle text-success-emphasis rounded-pill">BOM
                                                    Active</span>
                                            @else
                                                <span class="badge bg-danger-subtle text-danger-emphasis rounded-pill">BOM
                                                    Inactive</span>
                                            @endif
                                        </div>
                                        ...
                                        <div class="mt-2">
                                            <small class="text-muted">Can Produce: {{ $product->possible_units ?? 0 }}
                                                units</small>
                                        </div>
                                    </div>
                                    <div
                                        ...
                                        <a href="{{ route('bill_of_materials.show', $product->slug) }}"
                                            ...
                                            <i class="fas fa-eye me-1"></i> View BOM
                                        </a>
                                        @if (Auth::check() && Auth::user()->role && strtolower(Auth::user()->role->name) != 'staff')
